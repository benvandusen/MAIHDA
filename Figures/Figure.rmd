...
Jayson code
raw_plot_fun <- function(assessment){
raw_plot <- ggplot(data = plot_df %>% filter(assessment_code==assessment, gender_cat != "Other", race_cat!="Other"),
                           aes(y = Score, x = race_cat, fill = Time, color = Time))+
     scale_fill_viridis_d( option = "C")+ scale_color_viridis_d( option = "C")+
    geom_point( shape = 21,size=1, position = position_jitterdodge(),color="black",  alpha=.1)+
  ...
  xlab("")+
  ylab("Score (%)")+
  theme(legend.title = element_blank(), legend.position = 'bottom',legend.box = "horizontal",legend.direction = "horizontal") + facet_grid(gender_cat~.) #+
  #ggtitle(assessment)
return(raw_plot)
}
