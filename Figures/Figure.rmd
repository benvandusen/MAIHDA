...
Mean abs true error by sub-group sample size
```{r}
load("~/MAIHDA/Evaluation/SD_table.rda")

SD_table <- SD_table %>%
  mutate(N_fac = factor(ifelse(N_fac==500,"Total Sample = 500","Total Sample = 1000"), levels=c("Total Sample = 500","Total Sample = 1000")))

ggplot(data=filter(SD_table), aes(x=as.factor(N_groups), y=mean_abs)) +
